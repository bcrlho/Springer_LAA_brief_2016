...
\abstract*{In order to understand and analyze the interactions between U-LTE and Wi-Fi when sharing the $5$ GHz frequency band, knowledge on the architectures and Medium Access Control (MAC) mechanisms currently adopted by these two technologies is highly desired. This chapter focuses on the Wi-Fi technology and begins with an overview of the evolution of IEEE 802.11/Wi-Fi. Five existing generations, as well as the next generation, of IEEE 802.11/Wi-Fi are presented. The majority of this chapter provides ideas and detailed mechanisms of the CSMA/CA MAC protocol. Important observations on how CSMA/CA senses and occupies the radio medium when the LTE network is operating in vicinity are highlighted.}
...
Figs. \ref{figs:CSMA-CA-back-off-no-collision} and \ref{figs:CSMA-CA-back-off-with-collision} demonstrates the operations of the back-off procedure in two typical scenarios. 
\begin{figure}[!ht]
	\centering
